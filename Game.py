...
from Utils import *
import time
from players.ComputerAI import ComputerAI
from players.DanielAI import DanielAI
...
class Game():
    ...
    def throw(self, player, grid: Grid, intended_position: tuple):
        ...
        p = 1 - 0.05 * \
            (manhattan_distance(player.getPosition(), intended_position) - 1)
        ...
        while not self.over:
            ...
            if turn == 0:
                ...
                if self.is_valid_trap(self.grid, intended_trap):
                    ...
                    self.grid.trap(trap)
                    print(
                        f"Throwing a trap to: {intended_trap}. Trap landed in {trap}")
                ...
                if self.is_valid_trap(self.grid, intended_trap):
                    trap = self.throw(
                        self.computerAI, self.grid, intended_trap)
                    self.grid.trap(trap)
                    print(
                        f"Throwing a trap to: {intended_trap}. Trap landed in {trap}")
                else:
